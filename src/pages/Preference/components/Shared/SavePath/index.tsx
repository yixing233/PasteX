...
const SavePath: FC<{ state: SavePathState }> = (props) => {
  const { state } = props;
  ...
  useMount(async () => {
    setDefaultDataDir(await getDefaultSaveDataDir());
    setLogDir(await getDefaultLogDir());
  });
  ...
  const handleChange = async (isDefault = false) => {
    try {
      ...
      if (isDefault) {
        dstPath = defaultDataDir;
      } else {
        const dstDir = await open({ directory: true });

        if (!isString(dstDir)) return;

        dstPath = join(dstDir, getSaveDataDirName());
      }

      if (!dstPath || isEqualPath(dstPath)) return;
  ...
  const isEqualPath = (dstPath: string) => {
    ...
    <ProList header={t("preference.data_backup.storage_settings.title")}>
      ...
      >
        <Space.Compact>
          ...
          >
            <Button
              disabled={!defaultDataDir || isEqualPath(defaultDataDir)}
              icon={<ReloadOutlined />}
