...
import { LISTEN_KEY, REPOSITORY_LINK, UPDATE_MESSAGE_KEY } from "@/constants";
...
const UpdateApp = () => {
  ...
  const okText = useCreation(() => {
    const { loading, total, download, installing } = state;

    if (installing || (loading && total && download >= total)) {
      return t("component.app_update.button.installing");
    }
  ...
  }, [state.loading, state.total, state.download, state.installing]);
  ...
  const handleOk = async () => {
    ...
    try {
      await state.update.downloadAndInstall((event) => {
        switch (event.event) {
          case "Started":
            ...
            break;
          case "Progress":
            state.download += event.data.chunkLength;
            break;
          case "Finished":
      ...
      // Relaunch the application after the update is installed
        ...
        <Flex className="pt-1" gap="small" vertical>
          <Flex align="center">
            ...
            <span>
              ...
              <Link
                href={`${REPOSITORY_LINK}/releases/tag/${state.update?.version}`}
              >
